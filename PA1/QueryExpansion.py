...
class QueryExpansion(object):
    ...
    def replace_words_with_target_str(self, input_text, 
            replaced_words, target_str=" "):
        replaced_words_regex = re.compile(
            '|'.join(map(re.escape, replaced_words))
        )
        updated_text = replaced_words_regex.sub(target_str, input_text)
        return updated_text

    def replace_stop_words(self, input_text):
        return self.replace_words_with_target_str(input_text, stop_words)

    def replace_special_chars(self, input_text):
        return self.replace_words_with_target_str(input_text, special_chars)
    ...
    def build_input_text(self, res_json):
        input_res_list = [
                res_json[TITLE], 
                self.get_info_from_url(res_json[URL]),
                res_json[DESC]
        ]        
        input_text = ' '.join(input_res_list)
        input_text = self.replace_stop_words(input_text)
        input_text = self.replace_special_chars(input_text)
        return input_text
    ...
    def compute_tf_vector(self, input_text):
        word_stats = self.count_frequency(input_text)
        total_freq = sum(word_stats.values())
        tf_vector = []
        for word in self.all_words_vector:
            if word_stats.get(word):
                tf_vector.append(float(word_stats.get(word)) / total_freq)
            else:
                tf_vector.append(0)
        return tf_vector
    ...
    def compute_idf_vector(self, all_res_jsons):
        ...
        idf_vector = [1] * len(self.all_words_vector)
        for res_json in all_res_jsons:
            doc_str = self.build_input_text(res_json)
            for idx, word in enumerate(self.all_words_vector):
                #TODO::Some value will be larger than 10, dont know why.
                if word in doc_str:
                    print "word", word
                    print "in words vector", self.all_words_vector[idx]
                    if idf_vector[idx] < D:
                        idf_vector[idx] += 1
        print "idf_vector before", idf_vector
        for i in xrange(len(idf_vector)):
            idf_vector[i] = math.log(D / idf_vector[i], 2)
        print "all words", self.all_words_vector
    ...
    def get_tfidf_vectors(self, res_jsons):
        ...
        for res_json in res_jsons:
            ...
            tfidf_vector = self.com.dot_product(tf_vector, self.idf_vector)
    ...
    def compute_weight_vector(self, query, all_res_jsons, 
        ...
        # Rocchio's Algo
        relevant_tfidf_vectors_sum = self.com.vector_sum(
            relevant_tfidf_vectors
        )
        non_relevant_tfidf_vectors_sum = self.com.vector_sum(
            non_relevant_tfidf_vectors
        )
        query_vector = self.com.vector_multiply(
            query_vector,
            self.alpha
        )
        relevant_tfidf_vectors_sum = self.com.vector_multiply(
            relevant_tfidf_vectors_sum,
            self.beta / R
        )
        non_relevant_tfidf_vectors_sum = self.com.vector_multiply(
            non_relevant_tfidf_vectors_sum,
             -self.gama / NR
        )
        vectors = [query_vector, relevant_tfidf_vectors_sum, non_relevant_tfidf_vectors_sum]
        rocchio_query_vector = self.com.vector_sum(vectors)
