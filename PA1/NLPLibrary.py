...
class NLPLibrary(object):
    ...
    def replace_words_with_target_str(self, input_text, 
        ...
        replaced_words_regex =                                                 \
            re.compile('|'.join(map(re.escape, replaced_words)))
    ...
    def build_all_words(self, all_res_json):
        for res_json in all_res_json:
        ...
        self.idf_vector = self.compute_idf_vector(all_res_json)
    ...
    def compute_idf_vector(self, all_res_json):
        D = float(len(all_res_json))
    ...
    def compute_weight_vector(self, all_res_json):
        tfidf_vectors = []
        self.build_all_words(all_res_json)
        for res_json in all_res_json:
            input_text = self.build_input_text(res_json)
            tf_vector = self.compute_tf_vector(input_text)
            tfidf_vector = self.com.vector_multiply(tf_vector, self.idf_vector)
